...
"""
=================================================
File Name   ： ptb_word_lm.py.py
Author      :  jiang
Create Date ： 17-7-21 下午3:25
Description :  
-------------------------------------------------
Change Activity:
               17-7-21 下午3:25
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config):
        ...
        # lstm_cell = tf.contrib.rnn.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True) #原来的用法
        ...
        cell = tf.contrib.rnn.MultiRNNCell(
            [attn_cell() for _ in range(config.num_layers)], state_is_tuple=True) # 多层lstm cell 堆叠起来
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(num_steps):
                if time_step > 0: tf.get_variable_scope().reuse_variables()  #把这个scope范围内的参数都设置reuse=True
        ...
        self._lr_update = tf.assign(self._lr, self._new_lr)     #   使用new_lr来更新lr的值;这个是个op
...
if __name__=='__main__':
    ...
    config = get_config()  #参数在函数中由 FLAGS.model == "small"。。。决定
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        summary_writer = tf.summary.FileWriter('/home/jiang/PycharmProjects/TensorflowLearn/20170719复习tensorflow/lstm_logs',session.graph)
